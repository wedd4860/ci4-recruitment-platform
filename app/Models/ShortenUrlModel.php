...
class ShortenUrlModel extends Model
{
    ...
    protected $createdField = 'idx_shorten_url'; // 데이터 레코드 작성 타임스탬프를 유지하기 위해 사용하는 데이터베이스 필드
        ...
        if (is_array($this->fields)) {
            foreach ($this->fields as $key => $row) {
                if (isset($row['auto_increment']) && $row['auto_increment'] === true) {
                    $this->primaryKey = $key;
                } else if (!in_array($key, ['idx_shorten_url'])) {
    ...
    public function getSortUrl(string $code): array
    {
        $strResult = $this->where(['shorten_url' => $code])->orderBy('idx', 'desc')->first();
        return $strResult;
    }
}
